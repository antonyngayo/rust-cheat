...
impl FileNames {
    pub fn new(p: PathBuf) -> Self {
        Self {
            name: p.to_string_lossy().split("/").last().unwrap().to_string(),
            path: p.to_string_lossy().to_string(),
        }
    }
}
pub fn read_files(
    cheat_folder: &PathBuf,
) -> Result<HashMap<UniCase<String>, FileNames>, io::Error> {
    // returning a vector on success or an error on failure
...
pub fn perform_edit(binary: &PathBuf, file_path: PathBuf) {
    ...
    let _ = std::process::Command::new(binary)
        ...
        .spawn()
        .expect("An error occured while performing edit")
        .wait();
}
// open the files using the chosen editor; bash does the terminal spawning under the hood
...
pub fn choose_editor(
    ...
    stdin()
        .read_line(&mut editor_selection)
        .ok()
        .expect("An error occurred while capturing input"); // getting input from user
    match editor_selection.trim().parse::<usize>() {
        Ok(num) => {
            if num < selector.len() + 1 {
                editor_index = num - 1
            }
        }
